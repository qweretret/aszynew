package com.ossjk.config.redis;
...
public final class RedisUtil {
	...
	public void vagueDel(String... key) {
		if (key != null && key.length > 0) {
			...
			for (String keyStr : key) {
				...
				keys.addAll(redisTemplate.keys(keyStr));
			}
			this.del(keys.toArray(new String[0]));
		}
	}
	...
	public long incr(String key, long delta) {
		if (delta <= 0) {
			throw new QlhRunTimeException("递增因子必须大于0");
		}
	...
	public long decr(String key, long delta) {
		if (delta <= 0) {
			throw new QlhRunTimeException("递减因子必须大于0");
		}
		return redisTemplate.opsForValue().increment(key, -delta);
	}
