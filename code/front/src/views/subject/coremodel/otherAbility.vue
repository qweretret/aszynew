<template>
  <div>
    <h1>{{ messageInfo.name }}-{{ messageInfo.year }}-其它能力</h1>
    <div class="operator">
      <el-button type="primary" v-show="!isDisplay" @click="toOtherCP()">查看课时安排</el-button>
      <el-button type="primary"            v-has-permission-code-and="'10000800015'"       v-show="!isDisplay">清除当前专家意见</el-button>
      <el-button type="warning" @click="save()">{{ isDisplay ? "保存并返回" : "修改" }}</el-button>
      <el-button type="info"   @click="returnOther()">返回</el-button>
    </div>
    ...
    <other-course-plan ref="otherCoursePlan"></other-course-plan>
  </div>
</template>
...
import Vue from "vue";
import otherCoursePlan from "./otherCoursePlan.vue";
...
export default {
  ...
  watch: {
    messageInfo(val) {
      this.treeData = JSON.parse(val.model.otrmodel);
    },
  },
  mounted() {
    //消息主题
    ...
    this.$eventBus.$on("editEvent2", (data) => {
     ...
     if (data.col == 2) {
        ...
        optArr[idx*1].children[0].cntAll = data.input2;
        optArr[idx*1].children[0].cntSx = data.input3;
        optArr[idx*1].children[0].name = data.parameter.name2 ;
        optArr[idx*1].children[0].children[0].name = data.input2 + "(实操" + data.input3 + ")";
      }
    ...
    this.$eventBus.$on("addEvent2", (data) => {
      ...
      if (data.col == 1) {
       let newId= this.addNode(this.treeData, idArr[0], data.parameter.name1, "");
  ...
  methods: {
    //去查看课时安排
    toOtherCP() {
      this.$refs.otherCoursePlan.open(this.treeData, "查看课时安排");
    },
    save() {
       if (!this.isDisplay) {
        this.isDisplay = true;
      ...
      let param = {
        ...
        otrmodel: JSON.stringify(this.treeData),
      };
    ...
    list() {
      ...
      this.$nextTick(() => {
        if (
          this.messageInfo.model.otrmodel == null ||
          typeof this.messageInfo.model.otrmodel == undefined ||
          this.messageInfo.model.otrmodel == ""
        ) {
          this.treeData = [
            {
              id: "01",
          ...
          this.treeData = JSON.parse(this.messageInfo.model.otrmodel);
        }
        this.toColTreeData(this.treeData);
      });
    },
